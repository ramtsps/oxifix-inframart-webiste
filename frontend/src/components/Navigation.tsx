...
const Navigation = () => {
  ...
  const navItems = ["HOME", "ABOUT", "PRODUCTS", "INVESTORS"];
  ...
  return (
    ...
    >
            ...
            {navItems.map((item) => {
              ...
              return (
                <Link
                  key={item}
                  to={to}
                  className={`text-sm font-roboto font-bold transition-colors hover:text-primary ${isActive ? "text-primary" : "text-foreground"
                    }`}
                >
                  {item}
                </Link>
      ...
      {isMobileMenuOpen && (
        <div
          className={`lg:hidden absolute top-full left-0 w-full bg-background/95 backdrop-blur-md shadow-xl border-b border-border py-6 px-6 sm:px-8 ${isScrolled ? "mt-0" : "mt-2"
            }`}
        >
            ...
            {navItems.map((item) => {
              ...
              return (
                <Link
                  key={item}
                  to={to}
                  className={`text-sm font-medium py-3 px-4 rounded-lg transition-colors hover:bg-primary/10 ${isActive ? "text-primary bg-primary/10" : "text-foreground"
                    }`}
            ...
            <div className="pt-4 border-t border-border">
              <Link to="/contact" className="block">
                <button className="flex items-center justify-between rounded-full bg-primary text-white font-medium px-4 py-3 hover:scale-105 transition-transform duration-200 group">
                  <span className="pr-2 text-sm sm:text-base font-normal" >
                    CONTACT US
                  </span>
